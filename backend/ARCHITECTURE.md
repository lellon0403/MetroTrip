...
| `contract.py` | Bearer 인증, 공통 오류 응답, 미구현 응답 |
| `health.py` | 서버 상태 확인 |
| `auth.py` | 회원가입, 로그인, 토큰, 로그아웃 |
| `users.py` | 내 정보, 즐겨찾기, 내가 작성한 후기 |
| `transit.py` | 노선, 역, 시간표, 장소와 장소 관리 |
| `plans.py` | 여행 계획과 읽기 전용 공유 |
| `reviews.py` | 여행 후기와 미디어 업로드 계약 |
| `notices.py` | 공지사항 조회와 관리 |
| `community.py` | 일반 게시글, 모집 글과 참여 신청 |
...
현재는 API 계약 단계이므로 기본 패키지만 있으며, 실제 기능 구현 시 파일을 추가한다.
...
현재는 DB 모델 구현 전이므로 기본 패키지만 존재한다.

### `app/models/`

DB 명세 V1.8에 대응하는 SQLAlchemy 모델을 둔다.
...
노출하지 않는다. 현재는 V1.8 SQL과 ERD가 저장소에 반영된 후 구현할 예정이다.
...
기능 구현이 시작되면 대상 파일명을 따라 테스트를 추가한다.
