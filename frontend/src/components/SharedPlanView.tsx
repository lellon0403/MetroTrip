...
export function SharedPlanView({ plan, mapHref, backHref }: { plan: PlanView; mapHref?: string; backHref?: string }) {
  const hasDateRange = Boolean(plan.startDate && plan.endDate);
  return <main className="sharedPlan contentShell">
    ...
    <header>
      <p className="eyebrow">UNLISTED TRIP · 읽기 전용</p><h1>{plan.title}</h1><p>{hasDateRange ? `${ymd(plan.startDate)} – ${ymd(plan.endDate)}` : "일정 날짜 정보 없음"}</p>
    ...
    <div className="sharedDays">{plan.days.map((day, index) => <article key={day.id}><aside><span>DAY {index + 1}</span><strong>{day.title ?? `${index + 1}일차`}</strong>{day.dayDate ? <time>{ymd(day.dayDate)}</time> : null}</aside><ol>{day.items.map((item) => <li key={item.id}><small>{item.itemType}</small><strong>{item.note ?? (item.itemType === "PLACE" ? "여행 장소" : item.itemType === "STATION" ? "이동 역" : "저장된 경로")}</strong>{item.scheduledTime && <time>{item.scheduledTime.slice(0, 5)}</time>}</li>)}</ol></article>)}</div>
  </main>;
}
