...
## 2026-08-10 제품 경험 보강 실행 결과

Phase C~G의 기존 도메인을 재설계하지 않고 홈 aggregation, 맵 복수 필터·검색 ledger, 맵 일정 drawer, 단일 역 후기, 이미지 비율 metadata, 모집 feed 정렬, 이벤트 기간 필드를 추가했다. 신규 마이그레이션은 0009이며 OpenAPI TypeScript 계약을 다시 생성한다. 공개 도보 API로 오인하지 않도록 Kakao Mobility 도보 경로는 제휴 기능 flag 뒤에 두고 기본 로컬 모드는 명시적 예상값을 사용한다.
