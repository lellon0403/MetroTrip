...
export default async function ReviewDetailPage({ params }: { params: Promise<{ reviewId: string }> }) {
  ...
  return <main className="reviewDetail contentShell"><Link className="backLink" href="/reviews">← 후기 목록</Link><article><header><p className="eyebrow">TRAVEL REVIEW</p><h1>{review.title}</h1><div className="detailByline"><strong>{review.authorName}</strong><time>{review.travelDate}</time><span>조회 {review.viewCount}</span></div><div className="detailRoute"><b>{review.destinationStationId ? `${review.originStationName} → ${review.destinationStationName}` : `${review.originStationName}역`}</b><span>★ {review.rating}</span>{review.costWon !== null ? <span>{review.costWon.toLocaleString("ko-KR")}원</span> : null}</div><ReviewActions reviewId={review.id} authorId={review.authorId} title={review.title} /></header><div className="reviewDocument">{review.blocks.map((block, index) => block.kind === "PARAGRAPH" ? <p key={index}>{block.text}</p> : block.mediaId && media.get(block.mediaId) ? <figure key={index}><img src={media.get(block.mediaId)?.url} alt={block.altText ?? "여행 사진"} /></figure> : null)}</div><div className="detailTags">{review.tags.map((tag) => <span key={tag}>#{tag}</span>)}</div><ReviewLike reviewId={review.id} initialLiked={review.likedByMe} initialCount={review.likeCount} /></article></main>;
}
