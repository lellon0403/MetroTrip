...
> **2026-08-20 코드 검증:** V1.12 전체 동기화 실행 계획은 23개 테이블로 확인했고 관련 단위 테스트와 전체 148개 테스트가 통과했다. 현재 환경에서는 실제 Aiven MySQL·OCI Oracle 접속 재검증을 수행하지 못했으므로 위 2026-08-11 실환경 결과와 구분한다.
...
- 세션 분리, TIME/CLOB 변환, 전체 GET의 읽기 세션 사용, 앱 레벨 Oracle 쓰기 차단은 구현·자동 테스트로 확인했습니다.
- `test_db_failover.py`, `test_sync_to_oracle.py`를 포함한 백엔드 자동화 테스트는 2026-08-20 기준 148개가 통과했습니다.
- 2026-08-11 실환경에서는 Aiven MySQL과 OCI Oracle의 22개 테이블 동기화·조회 전환을 확인했습니다.
- V1.12 전체 23개 테이블 재적재, 별도 `metrotrip_ro` 계정의 DB 권한 차단, 동일 프로세스 내 장애 복구는 최신 실환경에서 재검증하지 않았습니다.
...
**당시 검증(2026-08-11)**: 단위 테스트 5건을 추가해 당시 전체 139개가 통과했고, 실제 앱의 스케줄러가 Aiven MySQL + OCI Oracle로 성공하는 것도 확인했습니다. **2026-08-20**에는 공유 프로브·`unavailable` 라우팅·Oracle 시간 문자열·23개 테이블 전체 동기화 회귀를 포함해 전체 148개가 통과했습니다.
