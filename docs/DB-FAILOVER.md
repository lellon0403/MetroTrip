...
| 파일 | 변경 |
|---|---|
| `backend/app/config.py` | `oracle_ro_url`, `oracle_sync_url`, `failover_cache_seconds`, `failover_fail_threshold`, `failover_recover_threshold`, `sync_interval_minutes` 추가 |
| `backend/.env.example` | `METROTRIP_ORACLE_RO_URL`, `METROTRIP_ORACLE_SYNC_URL` 키 추가 |
...
`oracledb`는 **thin 모드**로 동작하므로 Oracle Instant Client 설치가 필요 없습니다. 팀원 로컬 세팅에 별도 준비물이 없습니다.
...
| 4 | Oracle 버전 요건 | ⚠️ **환경 확인** | 제약 이름 30자 초과 6건 → **12.2 이상 필수**. 23ai Free는 충족 |
...
> **2026-08-11 로컬 실행 결과**: Oracle 미구성 환경이라 조회는 200 대신 `503 {"message":"일시적으로 조회 기능을 사용할 수 없습니다."}`로 확인(§6 `get_read_db()`의 의도된 동작 — Oracle 폴백 자체가 없으면 500 대신 503으로 안전하게 실패). `health/db`는 `{"routing":"oracle"}`, 쓰기는 `503 + Retry-After: 60`으로 정상 확인.
>
> **주의 — 서킷브레이커 전환 지연 구간**: 서비스 재기동 직후 `_state`가 `healthy=True`로 초기화되므로, 실제 중단 후 첫 프로브 실패(1회)만으로는 아직 전환되지 않는다(§6: 2회 연속 실패 필요). 이 짧은 전환 구간(캐시 주기 1회, 기본 5초)에 조회 요청이 들어오면 `get_read_db()`가 "MySQL 정상"으로 오판해 실제 MySQL 접속을 시도하다 처리되지 않은 예외로 `500`이 날 수 있음을 로컬 테스트에서 확인했다. 서킷브레이커가 완전히 전환된 뒤에는 위와 같이 정상적으로 503을 반환한다. 이는 "2회 연속 실패로 판정"이라는 §6의 의도된 트레이드오프(순간적 끊김에 흔들리지 않기 위함)의 부작용이며, 재기동 직후의 극히 짧은 창에서만 발생한다.
...
> **2026-08-11 로컬 실행 결과 — 미완료**: `net start MySQL80` 후 `health/db`가 계속 `oracle`로 남아 원인을 보니, MySQL 프로세스는 떠 있지만 `.env`의 root 비밀번호가 `1045 Access denied`로 거부되고 있었다(재기동 전에는 같은 자격 증명으로 정상 접속됐음). 페일오버 코드와는 무관한 **로컬 MySQL 인증 문제**로 판단해 이 부분 검증은 보류했다. 인증이 복구되면 이 항목만 다시 확인하면 된다.
...
- **엔진**: Oracle 23ai Free (RAM 2GB / CPU 2스레드 / 데이터 12GB 상한 — 본 프로젝트 규모에 충분하며, 상한이 있어 MySQL 메모리를 잠식하지 않는 이점이 있음)
...
- [ ] §9 항목 3·4·5 실환경 확인 (빈 문자열 0건 / Oracle 12.2+ / AL32UTF8) — **Oracle 인스턴스가 없어 미확인.** Oracle 설치 후 진행
- [ ] Oracle 계정 2개 생성, `metrotrip_ro`의 DML 거부 확인 (§10-6) — **Oracle 인스턴스가 없어 미확인**
- [x] §8.3 타입 변환 3종 구현 (TIME 문자열화 / CLOB 바인딩 / 빈 문자열 사전 점검) — 코드 구현 및 단위 테스트(SQLite) 통과. CLOB은 SQLAlchemy Core의 `Text` 타입 바인딩에 위임해 별도 `setinputsizes` 호출 없이 처리. **실제 Oracle 적재 검증은 미완료**
- [ ] `sync_to_oracle.py` 전체 재적재 성공, `--verify` 23개 테이블 일치 — 재적재·롤백·행 수 대조 로직은 SQLite 두 개를 오가며 단위 검증 완료(`test_sync_to_oracle.py`). **실제 MySQL→Oracle 적재는 미실행**
- [x] 전체 GET 엔드포인트가 `get_read_db()` 사용 (라우터 grep으로 누락 확인) — 완료
- [ ] §10 검증 8항목 전부 통과, 결과 PR 기록 — **10-2(MySQL 중단)·10-3(장애 중 동작) 로컬 실행·확인 완료**(결과는 §10-3에 기록). 10-4(복구)는 로컬 MySQL 인증 문제(1045)로 보류. Oracle이 필요한 10-1/10-5 실제 적재/10-6/10-7은 미완료
- [x] `test_db_failover.py`, `test_sync_to_oracle.py` 통과 — 기존 테스트(GET 엔드포인트 오버라이드 추가 포함) 합쳐 132개 전체 통과 확인
